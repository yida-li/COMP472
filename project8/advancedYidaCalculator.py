...
class MainApp(App):
    def build(self):
        ...
        self.answer = TextInput(
            multiline=False, readonly=True, halign="right", font_size=50, background_color=[168/255, 245/255, 200/255, 1]
        )
        ...
        self.answer2 = TextInput(
            multiline=False, readonly=True, halign="right", font_size=50, background_color=colors[2]
        )
    ...
    def on_button_press(self, instance):
        ...
        if button_chosen == "Clear Cache 3":
            ...
            self.answer4.text = ""
            return
        if button_chosen == "Erase":
            if self.answer.text:
                self.answer.text = self.answer.text.rstrip(
                    self.answer.text[-1])
            return
        if button_chosen == "log2":

            temp = self.answer.text
            rr = math.log2(float(temp))
            self.answer1.text = str(rr)
            return
        if button_chosen == "log10":

            temp = self.answer.text
            rr = math.log10(float(temp))
            self.answer1.text = str(rr)
            return
